...
def compress(file_paths, id):
    with tarfile.open(
        f"{ZIP_OUTPUT_PATH}/{ZIP_FILE_NAME}_{id}.tar.bz2", "w:bz2"
    ) as tar:
...
def multi_process_compress(dicom_subdirs_list):
    dicom_file_paths_groups = [
        [_get_file_paths(dicom_subdir) for dicom_subdir in dicom_subdirs]
        for dicom_subdirs in dicom_subdirs_list
    ]
